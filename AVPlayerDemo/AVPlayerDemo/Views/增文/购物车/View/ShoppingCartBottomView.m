...
- (void)changeShopcartBottomViewWithStatus:(BOOL)status {
    ...
    self.allSelectButton.hidden = !status;
}
...
- (void)renderWithTotalPrice:(NSString *)totalPrice {
    ...
    [attributedString addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:19]} range:[self.totalPriceLable.text rangeOfString:totalPrice]];
...
- (UIButton *)allSelectButton {
    if (_allSelectButton == nil){
        ...
        [_allSelectButton setImage:[UIImage imageNamed:@"未选中按钮-01"] forState:UIControlStateNormal];
        [_allSelectButton setImage:[UIImage imageNamed:@"单选按钮-选中"] forState:UIControlStateSelected];
