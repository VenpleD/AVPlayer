...
@interface VCWei () <UITableViewDelegate,UITableViewDataSource>
...
@implementation VCWei

- (NSMutableArray *)dataAry{
    if (!_dataAry) {
        
        _dataAry = [NSMutableArray arrayWithObjects:@{KTitle:@"定时器",KDetail:@"VCTimer"},
                                                    @{KTitle:@"视频播放",KDetail:@"VCPlayer"},
                                                    @{KTitle:@"RAC",KDetail:@"VCRAC"},
                                                    nil];
    }
    return _dataAry;
}
